...
/**
 * Metabox with images.
 * @todo It works but many issues is to be fixed. FIrst of all we need to merge it with ours meta class.
 * @todo Secondly — use more intuitive and pretty inteface for images.
 *
...
class Flotheme_Metabox {
	...
	function add_clone_script() {
		...
		jQuery(document).ready(function() {
			jQuery(".add").click(function() {
				var container = jQuery(this).parents(".newimages");
				jQuery("p:first-child", container).clone().insertAfter(container.find("p:last")).show();
				return false;
			});
	...
	function show() {
		...
		foreach ($this->_meta_box['fields'] as $field) {
			...
			echo '<tr '.($field['type'] == 'image' ? 'style="border-bottom:1px solid #CCC;"' : '').'>',
			...
			switch ($field['type']) {
				...
				case 'file':
					...
					break;
				case 'wysiwyg':
					wp_editor($meta ? $meta : $field['std'], $field['id'], array('media_buttons' => FALSE,'teeny' => TRUE));
					echo '<br />' ,$field['desc'];
					break;
				case 'image':
					?>
                    ...
                    <div class="newimages">
						...
						<a class="add" href="#">Добавить еще изображение</a>
					</div>
					<?php
					break;
			}
			...
			*/
			...
			if ($field['type'] == 'image') {
				if ($new && $old)
				{
					...
					$new = array_flip(explode(',', $old))+array_flip(explode(',', $new));
					$new = implode(',',array_keys($new));
				}
			}
			...
			{
				if ('' == $new && $old && $field['type'] != 'file' && $field['type'] != 'image')
